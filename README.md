...
## 🏗️ 全栈架构与完整目录结构 (Full Stack Architecture)

本项目采用**前后端分离 + 混合微服务架构**。前端负责现代化的多模态交互，Java 层（可选）作为轻量级 API 网关进行业务路由，Python 层作为核心 AI 引擎负责大模型调度与重度文件计算。

```text
📁 llm-paper-wiki-system/ (项目根目录)
│
├── 📁 frontend-vue/                  # 💻 前端：基于 Vue 3 + Element Plus 构建
│   ├── 📁 src/
│   │   ├── 📁 components/
│   │   │   ├── 📄 ChatView.vue       # 💬 核心聊天交互 UI：支持 Markdown、代码高亮、多轮 Agent 思考动画及沉淀触发
│   │   │   └── 📄 WikiView.vue       # 📖 知识库阅览 UI：渲染图文并茂的 Markdown 笔记与多模态架构图
│   │   ├── 📄 App.vue                # 页面主入口
│   │   └── 📄 main.js                # 全局注册配置
│   ├── 📄 package.json               # 前端依赖配置
│   └── 📄 vite.config.js             # 构建与前端代理配置
│
├── 📁 api-gateway-java/              # ☕ 中间件（可选）：基于 Spring Boot 构建的 API 网关
│   ├── 📁 src/main/java/.../
│   │   ├── 📁 controller/
│   │   │   └── 📄 WikiController.java # 接收前端请求并进行基础鉴权与转发
│   │   └── 📁 service/
│   │       └── 📄 WikiService.java    # 封装 RestTemplate/WebClient 向 Python 引擎发起 HTTP 请求
│   └── 📁 src/main/resources/
│       └── 📄 application.yml         # 跨域配置及 Python 引擎地址配置
│
├── 📁 ai-engine-python/              # 🧠 核心 AI 引擎：基于 FastAPI 与大模型构建
│   ├── 📄 config.py                  # ⚙️ 全局配置层：存放系统路径、API Keys (如 QWEN_API_KEY)
│   ├── 📄 utils.py                   # 🛠️ 通用工具层：文本预清洗、清洗参考文献等纯函数
│   ├── 📄 models.py                  # 📦 数据模型层：Pydantic 定义前后端交互 API 规范
│   ├── 📄 ai_client.py               # 🤖 模型驱动层：全局单例初始化 OpenAI/Qwen 客户端
│   ├── 📄 agent_tools.py             # 🧰 智能体工具箱：Agent 本地检索与深度阅读的具体动作执行与 Schema
│   ├── 📄 document_core.py           # 🏭 核心业务引擎：后台文献入库流水线、VLM 多模态视觉提取与智能鉴别过滤
│   ├── 📄 main.py                    # 🚦 极简入口路由：FastAPI 实例生命周期管理与 API 分发
│   ├── 📄 pdf_catalog.json           # 🗃️ 知识库总账本：记录所有 PDF 的路径、分类与极简摘要
│   └── 📁 temp_uploads/              # ⏳ 缓冲区：存放前端刚刚上传、等待后台管家处理的临时 PDF
│
└── 📁 Knowledge_Base/                # 🌱 本地动态生长的知识库物理目录 (受 Python 引擎管控)
    ├── 📁 PDF_Library/               # 📂 原始文献物理归档区
    │   ├── 📁 脑机接口/                 # 按 LLM 智能分类自动生成的文件夹
    │   │   └── 📄 FGANet.pdf         # 移动到此处的原始 PDF 文件
    │   └── 📁 大语言模型/
    │       └── 📄 ...
    ├── 📁 Wiki/                      # 🧠 Markdown 知识图谱核心区
    │   ├── 📄 index.md               # 动态更新的全库总索引，供用户快速总览
    │   ├── 📄 FGANet.md              # 经 LLM 结构化提取、翻译、并包含 AI Q&A 融合内容的最终笔记
    │   └── 📁 images/                # 🖼️ 本地图床：由 VLM 提取并过滤后的核心架构图、数据表
    │       └── 📄 FGANet_p3_1.png    # 命名规范关联至具体文献及页码
    └── 📄 settings.json              # 系统的动态用户配置持久化文件
